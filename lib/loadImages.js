...
export function loadParallaxPoster() {
  const images = require.context(
    "../public/img/poster", // Path to your parallax images
    false, // Don't include subdirectories
    /\.(webp|png|jpe?g|svg)$/, // Only include image files (webp, png, jpeg, svg)
  );
  ...
  return images.keys().map((key) => {
    ...
    const thumb = `/img/poster/${imageName}`; // You can modify this if you have separate thumbnails
    const caption = "test"; // Replace with dynamic captions if needed
...
export function loadMobileParallaxImages() {
  const images = require.context(
    "../public/img/mobileParallax", // Path to your parallax images
    false, // Don't include subdirectories
    /\.(webp|png|jpe?g|svg)$/, // Only include image files (webp, png, jpeg, svg)
  );
